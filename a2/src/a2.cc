...
int main (int argc, char *argv[])
{
  ...
  sinkApps.Stop(Seconds(60.0)); // Changed from 600s to 60s
  ...
  clientAppsA.Stop(Seconds(60.0)); // Changed from 600s to 60s
  ...
  clientAppsB.Stop(Seconds(60.0)); // Changed from 600s to 60s
  ...
  // Schedule movement at 20 seconds (adjusted from 10s)
  Simulator::Schedule(Seconds(20), &MoveUsers, 
  ...
  // Schedule movement at 40 seconds (adjusted from 15s)
  Simulator::Schedule(Seconds(40), &MoveUsers,
                     std::ref(staNodesA), std::ref(staNodesB),
                     std::ref(staDevsA), std::ref(staDevsB),
                     ssid1, ssid2, 0.50, 0.50, rng, std::ref(movementLog), "STA-A", "STA-B");
  
  // Log after second movement
  Simulator::Schedule(Seconds(40.1), &LogUserCounts, std::ref(staNodesA), std::ref(staNodesB));

  // Schedule periodic logging of throughput and user distribution - extend to 60s
  ...
  // Schedule periodic logging of user connections - extend to 60s
  for (double t = 1.0; t <= 60.0; t += 1.0) {
  ...
  Simulator::Schedule(Seconds(60.1), [&unifiedLog]() {
    unifiedLog.close();
  });
  ...
  // Set simulation duration to 60 seconds (increased from 20s)
